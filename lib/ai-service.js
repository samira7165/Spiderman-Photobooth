...
// =============================================================
// AI SERVICE — two swappable provider slots, each with a model
// fallback
// =============================================================
// Slot 1 is tried first, slot 2 is the fallback if slot 1 fails
// entirely. Within a slot, if the primary model fails, its
// fallback model is retried with the SAME key/provider before
// moving on to the next slot. Each slot independently picks which
// provider backs it (Gemini or OpenAI today). Settings are read
// fresh from the ApiSettings table on every call, falling back to
// .env if unset in the DB, so changes in the admin dashboard take
// effect immediately.
// =============================================================
...
async function getSlotSettings() {
  ...
  function resolveSlot(provider, apiKey, model, modelFallback) {
  ...
  return [
    resolveSlot(settings?.slot1Provider || "gemini", settings?.slot1ApiKey, settings?.slot1Model, settings?.slot1ModelFallback),
    resolveSlot(settings?.slot2Provider || "openai", settings?.slot2ApiKey, settings?.slot2Model, settings?.slot2ModelFallback),
  ];
}

/**
 * Generate an image by trying slot 1's model (then its fallback
 * model), then slot 2's model (then its fallback model).
...
export async function generateImage(prompt, userPhotoUrl, referenceImageUrl) {
  ...
  for (const slot of slots) {
    const modelsToTry = [slot.model, slot.modelFallback].filter(Boolean);

    for (const model of modelsToTry) {
      try {
        console.log(`[AI] Trying ${slot.provider} (${model})...`);
        const result = await dispatchGenerate({ ...slot, model }, prompt, userPhotoUrl, referenceImageUrl);
        return { imageBuffer: result, usedApi: slot.provider };
      } catch (err) {
        console.error(`[AI] ${slot.provider} (${model}) failed:`, err.message);
        errors.push(`${slot.provider}/${model}: ${err.message}`);
      }
    }
  }
